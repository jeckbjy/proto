...
namespace plugin_cpp
{
    class CppManagerWriter : IManagerWriter
    {
        ...
        void WriteHeader()
        {
            ...
            m_writer.WriteLine("struct PacketFactory");
            m_writer.WriteLine("{");
            m_writer.WriteLine("    pt_message* create(uint32_t msgid);");
            m_writer.WriteLine("};");
            m_writer.Close();
        }
        void WriteCpp()
        {
            ...
            // 写入函数
            m_writer.WriteLine();
            m_writer.WriteLine("pt_message* PacketFactory::create(uint32_t msgid){");
